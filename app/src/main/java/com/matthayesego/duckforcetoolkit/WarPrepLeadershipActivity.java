...
public class WarPrepLeadershipActivity extends Activity {
    ...
    private void load(){if(MemberPresentationPolicy.memberPreview(this)){renderError("War Prep Management is hidden in Standard Member Preview, matching normal member access.");return;}String key=keyStore.load();if(key==null||key.isBlank()){renderError("Reconnect your Torn API key first.");return;}new Thread(()->{try{session=TornApiClient.authenticateFreshFaction(key);if(!AccessPolicy.isLeaderPosition(session.position)){renderError("Faction Leader or Co-leader access is required.");return;}war=WarStatus.from(TornApiClient.getJson("/faction/wars",key),session.factionId);if(CommunityBackendClient.isConfigured()&&war.warId>0)shared=CommunityBackendClient.warPrepLeadership(key,war.warId);if(shared!=null&&shared.optJSONArray("items")!=null)items=shared.optJSONArray("items");else items=defaultItems();runOnUiThread(this::render);}catch(Exception e){renderError(e.getMessage()==null?"Unable to load faction War Prep.":e.getMessage());}},"TornFCA-WarPrepLeadership").start();}
