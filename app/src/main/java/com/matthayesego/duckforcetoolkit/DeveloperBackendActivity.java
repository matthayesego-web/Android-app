...
public class DeveloperBackendActivity extends Activity {
    ...
    private void load(){if(!DeveloperBackendClient.isConfigured())return;String key=keyStore.load();if(key==null||key.trim().isEmpty()){Toast.makeText(this,"Signed-in Torn API key is required.",Toast.LENGTH_LONG).show();return;}new Thread(()->{try{JSONObject response=DeveloperBackendClient.readConfig(key);JSONObject c=response.optJSONObject("config");config=c==null?new JSONObject():c;RemoteFeaturePolicy.applyVerifiedConfig(this,config);runOnUiThread(this::render);}catch(Exception e){String m=e.getMessage()==null?"Unable to load developer backend.":e.getMessage();runOnUiThread(()->{Toast.makeText(this,m,Toast.LENGTH_LONG).show();showLoading();});}},"TornFCA-DeveloperBackendLoad").start();}
