...
/** Owner-only UI for TornFCA's remote maintenance/version/feature policy. */
public class DeveloperBackendActivity extends Activity {
    ...
    private void load(){if(!DeveloperBackendClient.isConfigured())return;String key=keyStore.load();if(key==null||key.trim().isEmpty()){Toast.makeText(this,"Signed-in Torn API key is required.",Toast.LENGTH_LONG).show();return;}new Thread(()->{try{JSONObject response=DeveloperBackendClient.readConfig(key);JSONObject c=response.optJSONObject("config");config=c==null?new JSONObject():c;runOnUiThread(this::render);}catch(Exception e){String m=e.getMessage()==null?"Unable to load developer backend.":e.getMessage();runOnUiThread(()->{Toast.makeText(this,m,Toast.LENGTH_LONG).show();showLoading();});}},"TornFCA-DeveloperBackendLoad").start();}
    ...
    private void render(){ScrollView s=shell();LinearLayout root=base(s);TextView status=text("Verified remote policy • changes are server-side and audited",12,GREEN,true);status.setGravity(Gravity.CENTER);LinearLayout.LayoutParams st=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,ViewGroup.LayoutParams.WRAP_CONTENT);st.topMargin=dp(7);st.bottomMargin=dp(14);root.addView(status,st);
        ...
        add(root,card(BORDER));((TextView)((LinearLayout)root.getChildAt(root.getChildCount()-1)).getChildAt(0)).setText("");
        setContentView(s);s.requestApplyInsets();
    }

    private void save(String password,JSONObject next){if(password==null||password.isEmpty()){Toast.makeText(this,"Developer password required.",Toast.LENGTH_SHORT).show();return;}String key=keyStore.load();new Thread(()->{try{DeveloperBackendClient.writeConfig(key,password,next);RemoteFeaturePolicy.refreshAsync(this);runOnUiThread(()->{Toast.makeText(this,"Remote policy saved.",Toast.LENGTH_LONG).show();config=next;render();});}catch(Exception e){String m=e.getMessage()==null?"Unable to save remote policy.":e.getMessage();runOnUiThread(()->Toast.makeText(this,m,Toast.LENGTH_LONG).show());}},"TornFCA-DeveloperBackendSave").start();}
