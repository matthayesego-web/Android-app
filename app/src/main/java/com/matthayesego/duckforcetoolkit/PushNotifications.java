...
/** Optional FCM client. It is inert until Firebase is configured and the current legal version is acknowledged. */
public final class PushNotifications {
    ...
    public static String status(Context c){if(!firebaseConfigured())return"Cloud push is not configured in this build.";if(c!=null&&!LegalAcceptanceStore.hasAcceptedCurrent(c))return"Cloud push is waiting for the current legal acknowledgement.";if(!CommunityBackendClient.isConfigured())return"Firebase is ready; the TornFCA community backend is not connected.";String token=token(c);return token.isEmpty()?"Cloud push is configured; waiting for this device's FCM token.":"Cloud push is configured and this device has an FCM token.";}

    public static synchronized void initialize(Context context){
        if(initialized||context==null||!LegalAcceptanceStore.hasAcceptedCurrent(context)||!firebaseConfigured())return;
    ...
    public static void onNewToken(Context context,String value){if(context==null||!LegalAcceptanceStore.hasAcceptedCurrent(context)||value==null||value.isBlank())return;context.getSharedPreferences(PREFS,Context.MODE_PRIVATE).edit().putString(TOKEN,value.trim()).apply();syncIfReady(context);}
    ...
    public static void syncIfReady(Context context){
        if(context==null||!LegalAcceptanceStore.hasAcceptedCurrent(context))return;initialize(context);if(!cloudConfigured()||syncing)return;
    ...
    public static void requestCloudTest(Context context){if(context==null||!LegalAcceptanceStore.hasAcceptedCurrent(context)||!cloudConfigured())return;Context app=context.getApplicationContext();String key=new SecureApiKeyStore(app).load();if(key==null||key.isBlank())return;syncIfReady(app);new Thread(()->{try{CommunityBackendClient.sendPushTest(key);}catch(Exception ignored){}},"TornFCA-PushTest").start();}
