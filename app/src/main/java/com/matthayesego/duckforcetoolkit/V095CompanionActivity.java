...
public class V095CompanionActivity extends V090CompanionActivity {
    ...
    private void stampVersion(View view){if(view instanceof TextView){TextView t=(TextView)view;CharSequence raw=t.getText();if(raw!=null){String v=raw.toString().replace("v0.8.0","v"+VERSION).replace("v0.9.0","v"+VERSION).replace("v0.9.1","v"+VERSION).replace("v0.9.2","v"+VERSION).replace("v0.9.3","v"+VERSION).replace("v0.9.4","v"+VERSION).replace("v0.9.5","v"+VERSION).replace("v0.9.6","v"+VERSION).replace("v0.9.7","v"+VERSION).replace("v0.9.8","v"+VERSION).replace("v0.9.9","v"+VERSION).replace("v0.9.10","v"+VERSION).replace("v0.9.11","v"+VERSION).replace("v0.9.12","v"+VERSION).replace("v0.9.13","v"+VERSION);if(!v.equals(raw.toString()))t.setText(v);}}if(view instanceof ViewGroup){ViewGroup g=(ViewGroup)view;for(int i=0;i<g.getChildCount();i++)stampVersion(g.getChildAt(i));}}
