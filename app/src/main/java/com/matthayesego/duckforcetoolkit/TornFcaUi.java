...
public final class TornFcaUi {
    ...
    public static void header(Activity a,LinearLayout r,String backLabel,String title,String subtitle){Button back=button(a,"← "+backLabel,BORDER);back.setOnClickListener(v->a.finish());r.addView(back,new LinearLayout.LayoutParams(dp(a,132),dp(a,44)));TextView brand=text(a,"TORNFCA • FACTION COMPANION",10,GOLD,true);brand.setLetterSpacing(.11f);LinearLayout.LayoutParams bp=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,ViewGroup.LayoutParams.WRAP_CONTENT);bp.topMargin=dp(a,17);r.addView(brand,bp);r.addView(text(a,title,30,TEXT,true));TextView sub=text(a,subtitle,13,MUTED,false);LinearLayout.LayoutParams sp=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,ViewGroup.LayoutParams.WRAP_CONTENT);sp.topMargin=dp(a,4);sp.bottomMargin=dp(a,16);r.addView(sub,sp);}
