...
public class MoreActivity extends Activity {
    ...
    private void render(){ScrollView s=shell();LinearLayout r=new LinearLayout(this);r.setOrientation(LinearLayout.VERTICAL);s.addView(r,new ScrollView.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,ViewGroup.LayoutParams.WRAP_CONTENT));Button back=button("← Companion",BORDER);back.setOnClickListener(v->finish());r.addView(back,new LinearLayout.LayoutParams(dp(132),dp(44)));LinearLayout hero=new LinearLayout(this);hero.setOrientation(LinearLayout.VERTICAL);hero.setGravity(Gravity.CENTER_HORIZONTAL);hero.setPadding(dp(18),dp(18),dp(18),dp(18));hero.setBackground(gradient(Color.rgb(27,38,53),Color.rgb(11,16,23),BORDER,22));TextView brand=text("DUCK FORCE",10,GOLD,true);brand.setLetterSpacing(.16f);hero.addView(brand);TextView title=text("More",30,TEXT,true);hero.addView(title);TextView sub=text("Settings, premium roadmap and app information",13,MUTED,false);sub.setGravity(Gravity.CENTER);hero.addView(sub);LinearLayout.LayoutParams hp=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,ViewGroup.LayoutParams.WRAP_CONTENT);hp.topMargin=dp(14);hp.bottomMargin=dp(14);r.addView(hero,hp);LinearLayout premium=card("PREMIUM ROADMAP","Free vs Premium","Core faction work stays free. Premium is reserved for automation, deeper analytics, long history, exports and convenience—not basic faction participation or access.",GOLD);premium.setClickable(true);premium.setOnClickListener(v->startActivity(new Intent(this,PremiumPreviewActivity.class)));add(r,premium);LinearLayout about=card("APP INFORMATION","About Duck Force Companion","Purpose, version, data sources, privacy/security boundaries, FFScouter attribution and release status.",BLUE);about.setClickable(true);about.setOnClickListener(v->startActivity(new Intent(this,AboutActivity.class)));add(r,about);LinearLayout safety=card("SECURITY","Your data","Torn and FFScouter keys remain encrypted on-device. Shared faction systems should use faction_id as their tenant boundary and player_id as identity.",GREEN);add(r,safety);TextView footer=text("Duck Force Companion v0.9.7 • pre-release",11,MUTED,false);footer.setGravity(Gravity.CENTER);LinearLayout.LayoutParams fp=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,ViewGroup.LayoutParams.WRAP_CONTENT);fp.topMargin=dp(8);r.addView(footer,fp);setContentView(s);s.requestApplyInsets();}
}
