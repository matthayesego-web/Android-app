 ...
 *
 * Notices are additionally persisted per faction because they are intended to be broadly visible
 * and are safe to show from the last successful refresh while a new refresh is running. Banking
 * queue data stays in memory only so leadership queue visibility is never carried across a cold
 * process start without fresh backend authorization.
 */
public final class StartupWarmCache {
    private static final String PREFS = "tornfca_warm_cache_v1";
    private static final long NOTICE_PERSIST_MS = 6L * 60L * 60L * 1000L;
    ...
    public static synchronized void putSession(AuthSession value) {
        session = value;
        sessionFactionId = value == null ? 0 : value.factionId;
    }

    public static synchronized AuthSession session(int factionId) {
        return session != null && sessionFactionId == factionId ? session : null;
    }
    ...
    public static synchronized void putBanking(int factionId, int playerId, JSONObject value) {
        ...
        banking = copy(value);
        bankingAtMs = System.currentTimeMillis();
        bankingFactionId = factionId;
        bankingPlayerId = playerId;
    }
    ...
    public static synchronized void putWar(int factionId, JSONObject value) {
        ...
        war = copy(value);
        warAtMs = System.currentTimeMillis();
    ...
    public static void putNotices(Context context, int factionId, JSONArray value) {
        if (value == null || factionId <= 0) return;
        JSONArray safe = copy(value);
        long now = System.currentTimeMillis();
        synchronized (StartupWarmCache.class) {
            notices = safe;
            noticesAtMs = now;
            noticesFactionId = factionId;
        }
        if (context != null) {
    ...
    public static JSONArray notices(Context context, int factionId, long maxAgeMs) {
        synchronized (StartupWarmCache.class) {
            if (notices != null && noticesFactionId == factionId
                    && (maxAgeMs <= 0L || System.currentTimeMillis() - noticesAtMs <= maxAgeMs)) {
                return copy(notices);
            }
        }
    ...
    public static JSONObject latestVisibleNotice(Context context, int factionId) {
        JSONArray rows = notices(context, factionId, NOTICE_PERSIST_MS);
        if (rows == null) return null;
        long now = System.currentTimeMillis() / 1000L;
        String dismissed = context == null ? "" : context.getApplicationContext()
                .getSharedPreferences(PREFS, Context.MODE_PRIVATE).getString("dismissed_notice_id", "");
        for (int i = 0; i < rows.length(); i++) {
            JSONObject row = rows.optJSONObject(i);
            if (row == null) continue;
            long expires = row.optLong("expires_at", 0L);
            if (expires > 0L && expires <= now) continue;
            String id = row.optString("id", "");
            if (!id.isEmpty() && id.equals(dismissed)) continue;
            return copy(row);
        }
        return null;
    }
    ...
    public static void dismissNotice(Context context, String id) {
        ...
        context.getApplicationContext().getSharedPreferences(PREFS, Context.MODE_PRIVATE).edit()
                .putString("dismissed_notice_id", id)
                .apply();
        FactionAnnouncementOverlay.refreshVisible();
    }
    ...
    private static JSONObject copy(JSONObject value) {
        ...
        try { return new JSONObject(value.toString()); } catch (Exception e) { return null; }
    }

    private static JSONArray copy(JSONArray value) {
        if (value == null) return null;
        try { return new JSONArray(value.toString()); } catch (Exception e) { return null; }
    }
}
