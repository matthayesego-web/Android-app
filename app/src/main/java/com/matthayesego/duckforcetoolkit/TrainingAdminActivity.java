...
/** Leader/Co-leader editor for faction-scoped training expectations and guide library. */
public class TrainingAdminActivity extends Activity {
    ...
    private void showLoading(){ScrollView s=TornFcaUi.shell(this);LinearLayout r=TornFcaUi.root(this,s);TornFcaUi.header(this,r,"Training Center","Training Management","Verifying faction leadership and loading current rules…");TornFcaUi.add(this,r,TornFcaUi.card(this,"LEADERSHIP","Loading training management","All writes are re-verified by the faction-scoped community backend.",TornFcaUi.GOLD));setContentView(s);s.requestApplyInsets();}

    private void load(){String key=keyStore.load();if(key==null||key.isBlank()){renderError("Reconnect your Torn API key first.");return;}if(!CommunityBackendClient.isConfigured()){renderError("The TornFCA community backend is not configured in this build.");return;}new Thread(()->{try{AuthSession verified=TornApiClient.cachedSession(key);if(verified==null)verified=TornApiClient.authenticate(key);if(!isLeader(verified.position))throw new Exception("Training management is currently restricted to the faction Leader and Co-leader.");session=verified;library=CommunityBackendClient.trainingLibrary(key);runOnUiThread(this::render);}catch(Exception e){renderError(e.getMessage()==null?"Unable to load training management.":e.getMessage());}},"TornFCA-TrainingAdmin").start();}
    ...
    private void render(){ScrollView s=TornFcaUi.shell(this);LinearLayout r=TornFcaUi.root(this,s);TornFcaUi.header(this,r,"Training Center","Training Management",session.factionName+" • Leader/Co-leader publishing");
        JSONObject rules=library.optJSONObject("trainingRules");
        TornFcaUi.addSection(this,r,"FACTION EXPECTATIONS");LinearLayout rc=TornFcaUi.card(this,"RULES","Set current training expectations","Use plain-language targets so every member can understand what the faction expects. Examples: “5% total battle-stat gain per month” or “Aim for 2–3 Xanax per day when not stacking for war.”",TornFcaUi.GOLD);
        ...
        TornFcaUi.addSection(this,r,"PUBLISH A GUIDE");LinearLayout gc=TornFcaUi.card(this,"FACTION LIBRARY","Add faction-specific training guide","Guides published here belong to this faction ID. Members of other factions cannot read them, and the guide does not follow a player who leaves.",TornFcaUi.BLUE);EditText title=field("Guide title",false),category=field("Category (Happy Jump, New Player, War Prep…)",false),body=field("Guide content",true);addField(gc,title,48);addField(gc,category,48);addField(gc,body,0);Button publish=TornFcaUi.button(this,"Publish guide",TornFcaUi.BLUE);LinearLayout.LayoutParams pp=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,TornFcaUi.dp(this,46));pp.topMargin=TornFcaUi.dp(this,10);gc.addView(publish,pp);publish.setOnClickListener(v->publishGuide(title.getText().toString(),category.getText().toString(),body.getText().toString()));TornFcaUi.add(this,r,gc);

        TornFcaUi.addSection(this,r,"CURRENT FACTION GUIDES");JSONArray guides=library.optJSONArray("guides");if(guides==null||guides.length()==0)TornFcaUi.add(this,r,TornFcaUi.card(this,"LIBRARY","No guides yet","Publish the first faction-specific training guide above.",TornFcaUi.BORDER));else for(int i=0;i<guides.length();i++){JSONObject g=guides.optJSONObject(i);if(g!=null)addExistingGuide(r,g);}
        r.addView(TornFcaUi.footer(this,"Server-side faction identity is rechecked for every training write."));setContentView(s);s.requestApplyInsets();}
    ...
    private void publishGuide(String title,String category,String body){if(title.trim().isEmpty()||body.trim().isEmpty()){Toast.makeText(this,"Guide title and content are required.",Toast.LENGTH_SHORT).show();return;}String key=keyStore.load();new Thread(()->{try{CommunityBackendClient.saveTrainingGuide(key,"",title,category,body);library=CommunityBackendClient.trainingLibrary(key);runOnUiThread(()->{Toast.makeText(this,"Guide published.",Toast.LENGTH_SHORT).show();render();});}catch(Exception e){toast(e);}},"TornFCA-PublishTrainingGuide").start();}
    private void archiveGuide(String id){String key=keyStore.load();new Thread(()->{try{CommunityBackendClient.archiveTrainingGuide(key,id);library=CommunityBackendClient.trainingLibrary(key);runOnUiThread(()->{Toast.makeText(this,"Guide archived.",Toast.LENGTH_SHORT).show();render();});}catch(Exception e){toast(e);}},"TornFCA-ArchiveTrainingGuide").start();}
    private void toast(Exception e){runOnUiThread(()->Toast.makeText(this,e.getMessage()==null?"Training update failed.":e.getMessage(),Toast.LENGTH_LONG).show());}
