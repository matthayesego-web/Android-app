...
/**
 * Central faction banking request queue.
 *
 * v0.10.11 deliberately keeps the opening path simple: render one safe cached snapshot, then
 * refresh the queue and balances independently. A stale/malformed queue row is never allowed to
 * crash the Activity.
 */
public class BankingCompanionActivity extends Activity {
    ...
    private static final long QUEUE_CACHE_MS=45L*60L*1000L;
    ...
    private boolean factionApiAccess=false,canManage=false,refreshing=false,balancesLoading=false,destroyed=false;
    ...
    @Override protected void onCreate(Bundle b){
        ...
        try{loadQueue();}catch(RuntimeException e){renderUnavailable("Banking recovered from a display error. Tap Retry to reload the queue.");}
    }
    ...
    private TextView text(String value,float size,int color,boolean bold){TextView t=new TextView(this);t.setText(value==null?"":value);t.setTextSize(size);t.setTextColor(color);t.setLineSpacing(0f,1.08f);if(bold)t.setTypeface(Typeface.DEFAULT,Typeface.BOLD);return t;}
    ...
    private void header(LinearLayout r){
        Button back=button("← Faction",BORDER);back.setOnClickListener(v->finish());r.addView(back,new LinearLayout.LayoutParams(dp(125),dp(44)));
        TextView e=eyebrow("FACTION OPERATIONS • BANKING",BLUE);LinearLayout.LayoutParams ep=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,ViewGroup.LayoutParams.WRAP_CONTENT);ep.topMargin=dp(18);r.addView(e,ep);
        r.addView(text("Banking Companion",30,TEXT,true));
        TextView sub=text("Request money, see the shared queue quickly, and jump straight into Torn when a payout needs action.",13,MUTED,false);LinearLayout.LayoutParams sp=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,ViewGroup.LayoutParams.WRAP_CONTENT);sp.topMargin=dp(5);sp.bottomMargin=dp(16);r.addView(sub,sp);
    }
    ...
    private void loadQueue(){
        ...
        boolean renderedWarm=false;
        if(playerId>0&&factionId>0){
            JSONObject warm=StartupWarmCache.banking(factionId,playerId,QUEUE_CACHE_MS);
            if(warm!=null){
                try{applyResponse(warm);renderSafe();renderedWarm=true;}catch(RuntimeException ignored){requests=new JSONArray();}
            }
        }
        if(!renderedWarm)showLoading("Loading the shared banking queue…");
        refreshLive(key,!renderedWarm);
    }
        ...
        new Thread(()->{
            try{
                ...
                runOnUiThread(()->{
                    ...
                    renderSafe();
                    loadBalancesAsync(key);
                });
            }catch(Exception e){
                String m=e.getMessage()==null?"Unable to load the banking queue.":e.getMessage();
                if(destroyed)return;
                runOnUiThread(()->{
                    if(destroyed)return;
                    refreshing=false;
                    if(firstLoad)renderUnavailable(m);else Toast.makeText(this,"Queue refresh failed: "+m,Toast.LENGTH_LONG).show();
                });
            }
        },"TornFCA-BankingQueue").start();
    }
    ...
    private void applyResponse(JSONObject response){
        ...
        JSONObject user=response.optJSONObject("user");
        if(user!=null){
            int uid=user.optInt("id",0),fid=user.optInt("faction_id",0);if(uid>0)playerId=uid;if(fid>0)factionId=fid;
            String un=user.optString("name","");if(!un.isBlank())playerName=un;
            String fn=user.optString("faction_name","");if(!fn.isBlank())factionName=fn;
            String pos=user.optString("position","");if(!pos.isBlank())position=pos;
        }
        boolean memberPreview=MemberPresentationPolicy.memberPreview(this);
        canManage=!memberPreview&&response.optBoolean("can_manage",false);
        ...
        if(canManage&&!factionApiAccess&&balanceError.isEmpty())balanceError="Detailed faction balances are not available to this key. Amount requests still work, and members can resolve their own full balance when submitting.";
    }
    ...
    private void loadBalancesAsync(String key){
        if(!canManage||!factionApiAccess||balancesLoading||destroyed)return;
        balancesLoading=true;
        new Thread(()->{
            String error="";Map<Integer,Long> fresh=new HashMap<>();
            try{
                ...
                for(int i=0;members!=null&&i<members.length();i++){JSONObject m=members.optJSONObject(i);if(m==null)continue;int id=m.optInt("id",0);if(id>0)fresh.put(id,Math.max(0L,m.optLong("money",0L)));}
            }catch(Exception e){error=e.getMessage()==null?"Current faction balances could not be loaded.":e.getMessage();}
            String finalError=error;
            if(destroyed)return;
            runOnUiThread(()->{
                if(destroyed)return;
                balancesLoading=false;balances.clear();balances.putAll(fresh);balanceError=finalError;renderSafe();
            });
    ...
    private void renderSafe(){
        ...
        try{render();}
        catch(RuntimeException e){renderUnavailable("A banking queue item could not be displayed safely. The queue can still be refreshed without closing TornFCA.");}
    }

    private void render(){
        ScrollView s=shell();LinearLayout r=root(s);header(r);addRequestCard(r);
        LinearLayout status=card(canManage?GREEN:BLUE);
        status.addView(eyebrow(canManage?"BANKER ACCESS":"MEMBER VIEW",canManage?GREEN:BLUE));
        status.addView(text(canManage?"Shared queue + payout controls":"Your banking requests",19,TEXT,true));
        status.addView(text(canManage?"Cached queue data opens first. Live queue and balance refreshes happen separately so one slow request cannot block the screen.":"Submit a request here. Authorized faction bankers receive it in the shared queue.",12.5f,MUTED,false));
        if(!balanceError.isEmpty())status.addView(text(balanceError,11.5f,RED,false));
        ...
        if(requests.length()==0){LinearLayout none=card(BORDER);none.addView(text("No banking requests yet.",17,TEXT,true));none.addView(text("Requests you submit in TornFCA will appear here.",12,MUTED,false));add(r,none);}else{
            for(int i=0;i<requests.length();i++){
                JSONObject row=requests.optJSONObject(i);if(row==null)continue;
                try{add(r,requestCard(row));}
                catch(RuntimeException rowError){LinearLayout bad=card(RED);bad.addView(text("Banking request could not be displayed",15,TEXT,true));bad.addView(text("This request was skipped instead of closing the app. Refresh the queue to retry it.",11.5f,MUTED,false));add(r,bad);}
            }
        }
    ...
    private void addRequestCard(LinearLayout r){
        LinearLayout c=card(GOLD);c.addView(eyebrow("NEW REQUEST",GOLD));c.addView(text("Request a faction payout",19,TEXT,true));c.addView(text("Leave the amount blank to request your full current faction balance. TornFCA will try to resolve that balance at submission time.",12,MUTED,false));
        ...
        submit.setOnClickListener(v->{
            ...
            String raw=amount.getText().toString().trim(),noteValue=note.getText().toString().trim();
            submit.setEnabled(false);submit.setText(raw.isEmpty()?"Resolving full balance…":"Submitting…");
            new Thread(()->{
                try{
                    ...
                    JSONObject response=CompanionBackendClient.submitBankingRequest(key,sendAmount,noteValue);
                    JSONObject request=response.optJSONObject("request");long finalResolved=resolved;
                    if(destroyed)return;
                    runOnUiThread(()->{if(destroyed)return;if(request!=null)addRequestLocally(request);Toast.makeText(this,requestedFull&&finalResolved>0L?"Full-balance request queued for "+money(finalResolved)+".":"Banking request queued.",Toast.LENGTH_SHORT).show();renderSafe();});
                ...
                }catch(Exception e){String m=e.getMessage()==null?"Unable to submit banking request.":e.getMessage();if(destroyed)return;runOnUiThread(()->{if(destroyed)return;submit.setEnabled(true);submit.setText("Submit Banking Request");Toast.makeText(this,m,Toast.LENGTH_LONG).show();});}
            },"TornFCA-BankingSubmit").start();
        });
        add(r,c);
    }
    ...
    private void addRequestLocally(JSONObject request){if(request==null)return;JSONArray next=new JSONArray();next.put(request);String id=request.optString("id","");for(int i=0;i<requests.length();i++){JSONObject row=requests.optJSONObject(i);if(row==null||(!id.isEmpty()&&id.equals(row.optString("id",""))))continue;next.put(row);}requests=next;cacheCurrentQueue();}
    ...
    private LinearLayout requestCard(JSONObject row){
        int requesterId=row.optInt("requester_id",0);String name=row.optString("requester_name","Member");String status=row.optString("status","PENDING").toUpperCase(Locale.US);String source=row.optString("source","TORNFCA");if(source==null||source.isBlank())source="TORNFCA";
        boolean amountMode=!row.isNull("requested_amount");long requested=amountMode?Math.max(0L,row.optLong("requested_amount",0L)):0L;Long current=balances.get(requesterId);Long due=amountMode?requested:current;
        ...
        String moneyLine=amountMode?"Requested "+money(requested):"Requested FULL BALANCE";if(current!=null)moneyLine+=" • current balance "+money(current);c.addView(text(moneyLine,12.5f,MUTED,false));
        String note=row.optString("note","");String chat=row.optString("request_text","");if(!note.isBlank())c.addView(text(note,11.5f,TEXT,false));else if(!chat.isBlank())c.addView(text(chat,11.5f,TEXT,false));
    ...
    private void updateStatus(String requestId,String status){String key=keyStore.load();if(key==null||key.isBlank())return;new Thread(()->{try{JSONObject response=CompanionBackendClient.updateBankingRequest(key,requestId,status);JSONObject updated=response.optJSONObject("request");if(destroyed)return;runOnUiThread(()->{if(destroyed)return;if(updated!=null)replaceRequestLocally(updated);Toast.makeText(this,"Banking request updated.",Toast.LENGTH_SHORT).show();renderSafe();});}catch(Exception e){String m=e.getMessage()==null?"Unable to update banking request.":e.getMessage();if(destroyed)return;runOnUiThread(()->Toast.makeText(this,m,Toast.LENGTH_LONG).show());}},"TornFCA-BankingUpdate").start();}
