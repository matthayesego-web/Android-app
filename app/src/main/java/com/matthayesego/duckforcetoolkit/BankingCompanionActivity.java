...
/** Central faction banking request queue with Torn payment handoff. */
public class BankingCompanionActivity extends Activity {
    ...
    private String factionName="Faction",position="Member";
    private boolean factionApiAccess=false,canManage=false;
    ...
    private void loadQueue(){
        if(!CompanionBackendClient.isConfigured()){renderUnavailable("The shared faction backend is not configured in this build yet. Native/local banking remains available in the legacy layer until the faction backend URL is supplied at build time.");return;}
    ...
    private void loadBalances(String key){
        ...
        try{
            ...
            for(int i=0;members!=null&&i<members.length();i++){JSONObject m=members.optJSONObject(i);if(m==null)continue;balances.put(m.optInt("id",0),Math.max(0L,m.optLong("money",0L)));}
        }catch(Exception e){balanceError=e.getMessage()==null?"Current faction balances could not be loaded.":e.getMessage();}
    }
    ...
    private void addRequestCard(LinearLayout r){
        LinearLayout c=card(GOLD);c.addView(eyebrow("NEW REQUEST",GOLD));c.addView(text("Request a faction payout",19,TEXT,true));c.addView(text("Leave the amount blank to request your full current faction balance.",12,MUTED,false));
        ...
        submit.setOnClickListener(v->{String key=keyStore.load();if(key==null)return;submit.setEnabled(false);submit.setText("Submitting…");new Thread(()->{try{CompanionBackendClient.submitBankingRequest(key,amount.getText().toString(),note.getText().toString());runOnUiThread(()->{Toast.makeText(this,"Banking request queued.",Toast.LENGTH_SHORT).show();loadQueue();});}catch(Exception e){String m=e.getMessage()==null?"Unable to submit banking request.":e.getMessage();runOnUiThread(()->{submit.setEnabled(true);submit.setText("Submit Banking Request");Toast.makeText(this,m,Toast.LENGTH_LONG).show();});}}).start();});
        add(r,c);
    }
    ...
    private LinearLayout requestCard(JSONObject row){
        ...
        if(canManage&&"PENDING".equals(status)){
            ...
            else{TextView unresolved=text("Full-balance amount is waiting for a readable faction balance.",11.5f,RED,false);LinearLayout.LayoutParams up=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,ViewGroup.LayoutParams.WRAP_CONTENT);up.topMargin=dp(9);c.addView(unresolved,up);}
    ...
    private void updateStatus(String requestId,String status){String key=keyStore.load();if(key==null)return;showLoading("Updating banking request…");new Thread(()->{try{CompanionBackendClient.updateBankingRequest(key,requestId,status);runOnUiThread(this::loadQueue);}catch(Exception e){String m=e.getMessage()==null?"Unable to update banking request.":e.getMessage();runOnUiThread(()->renderUnavailable(m));}}).start();}

    private void openTornPayment(int playerId,long amount){String url="https://www.torn.com/factions.php?step=your#/tab=controls&option=give-to-user&giveMoneyTo="+playerId+"&money="+amount;try{startActivity(new Intent(Intent.ACTION_VIEW,Uri.parse(url)));}catch(Exception e){Toast.makeText(this,"Unable to open Torn payout controls.",Toast.LENGTH_SHORT).show();}}
