...
/** Hidden triple-tap developer entry. Requires both the configured password and verified owner identity. */
public class DeveloperGateActivity extends Activity {
    ...
    private void renderPassword(String error){
        ...
        TextView brand=text("TORNFCA • HIDDEN DEVELOPER CHANNEL",11,GOLD,true);brand.setLetterSpacing(.12f);brand.setGravity(Gravity.CENTER);r.addView(brand);
        ...
        LinearLayout card=new LinearLayout(this);card.setOrientation(LinearLayout.VERTICAL);card.setPadding(dp(18),dp(18),dp(18),dp(18));card.setBackground(rounded(PANEL,BLUE,18));card.addView(text("Developer password",19,TEXT,true));
        TextView body=text("This console is intentionally absent from normal navigation. Access requires the configured developer password and the verified Torn identity assigned to the app owner.",13,MUTED,false);LinearLayout.LayoutParams bp=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,ViewGroup.LayoutParams.WRAP_CONTENT);bp.topMargin=dp(6);card.addView(body,bp);
        ...
        TextView status=text(error==null?"Triple tap footer + developer password + verified owner account":error,12,error==null?MUTED:BAD,false);status.setGravity(Gravity.CENTER);LinearLayout.LayoutParams st=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,ViewGroup.LayoutParams.WRAP_CONTENT);st.topMargin=dp(9);card.addView(status,st);r.addView(card,new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,ViewGroup.LayoutParams.WRAP_CONTENT));
        unlock.setOnClickListener(v->{String entered=pass.getText().toString();if(!BuildConfig.DEVELOPER_ACCESS_SHA256.equals(sha256(entered))){status.setText("Incorrect developer password.");status.setTextColor(BAD);pass.setText("");return;}resolveContextAndOpen();});
        setContentView(s);s.requestApplyInsets();
    }

    private void resolveContextAndOpen(){
        String key=keyStore.load();
        if(key==null||key.trim().isEmpty()){renderPassword("Reconnect your Torn API key before opening developer controls.");return;}
        FactionScopeCache.Scope cached=FactionScopeCache.load(this,key);
        if(cached!=null){
            if(cached.playerId!=BuildConfig.DEVELOPER_PLAYER_ID){renderPassword("Developer controls are restricted to the verified owner account.");return;}
            openPanel(cached.factionId,cached.factionName,cached.position,cached.factionApiAccess);return;
        }
        new Thread(()->{
            try{
                AuthSession session=TornApiClient.authenticate(key);FactionScopeCache.save(this,key,session);
                runOnUiThread(()->{
                    if(session.playerId!=BuildConfig.DEVELOPER_PLAYER_ID){renderPassword("Developer controls are restricted to the verified owner account.");return;}
                    openPanel(session.factionId,session.factionName,session.position,session.factionApiAccess);
                });
            }catch(Exception e){runOnUiThread(()->renderPassword("Unable to verify the developer account. Reconnect and try again."));}
        },"TornFCA-DeveloperIdentity").start();
    }

    private void openPanel(int factionId,String factionName,String position,boolean factionApiAccess){Intent i=new Intent(this,DeveloperPanelActivity.class);i.putExtra(DeveloperConsoleActivity.EXTRA_FACTION_ID,factionId);i.putExtra(DeveloperConsoleActivity.EXTRA_FACTION_NAME,factionName);i.putExtra(DeveloperConsoleActivity.EXTRA_FACTION_API,factionApiAccess);i.putExtra(DeveloperConsoleActivity.EXTRA_POSITION,position);startActivity(i);finish();}
