...
/** Hidden triple-tap developer entry. Password is build-configured; Torn identity is only context. */
public class DeveloperGateActivity extends Activity {
    ...
    private void renderPassword(String error){ScrollView s=shell();LinearLayout r=new LinearLayout(this);r.setOrientation(LinearLayout.VERTICAL);r.setGravity(Gravity.CENTER_HORIZONTAL);s.addView(r,new ScrollView.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,ViewGroup.LayoutParams.WRAP_CONTENT));TextView brand=text("TORNFCA • HIDDEN DEVELOPER CHANNEL",11,GOLD,true);brand.setLetterSpacing(.12f);brand.setGravity(Gravity.CENTER);r.addView(brand);TextView title=text("Developer Console",30,TEXT,true);title.setGravity(Gravity.CENTER);LinearLayout.LayoutParams tp=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,ViewGroup.LayoutParams.WRAP_CONTENT);tp.topMargin=dp(7);tp.bottomMargin=dp(18);r.addView(title,tp);LinearLayout card=new LinearLayout(this);card.setOrientation(LinearLayout.VERTICAL);card.setPadding(dp(18),dp(18),dp(18),dp(18));card.setBackground(rounded(PANEL,BLUE,18));card.addView(text("Developer password",19,TEXT,true));TextView body=text("This console is intentionally absent from normal navigation. Anyone entrusted with the preconfigured developer password can enter it from the hidden triple-tap gesture.",13,MUTED,false);LinearLayout.LayoutParams bp=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,ViewGroup.LayoutParams.WRAP_CONTENT);bp.topMargin=dp(6);card.addView(body,bp);EditText pass=new EditText(this);pass.setHint("Developer password");pass.setHintTextColor(MUTED);pass.setTextColor(TEXT);pass.setSingleLine(true);pass.setInputType(InputType.TYPE_CLASS_TEXT|InputType.TYPE_TEXT_VARIATION_PASSWORD);pass.setPadding(dp(14),0,dp(14),0);pass.setBackground(rounded(BG,BORDER,12));LinearLayout.LayoutParams pp=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,dp(54));pp.topMargin=dp(15);card.addView(pass,pp);Button unlock=new Button(this);unlock.setText("Open Developer Panel");unlock.setAllCaps(false);unlock.setTextColor(Color.rgb(24,17,8));unlock.setTextSize(15);unlock.setTypeface(Typeface.DEFAULT,Typeface.BOLD);unlock.setBackground(rounded(GOLD,GOLD,12));LinearLayout.LayoutParams up=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,dp(50));up.topMargin=dp(11);card.addView(unlock,up);TextView status=text(error==null?"Triple tap footer + configured password":error,12,error==null?MUTED:BAD,false);status.setGravity(Gravity.CENTER);LinearLayout.LayoutParams st=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,ViewGroup.LayoutParams.WRAP_CONTENT);st.topMargin=dp(9);card.addView(status,st);r.addView(card,new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,ViewGroup.LayoutParams.WRAP_CONTENT));unlock.setOnClickListener(v->{String entered=pass.getText().toString();if(!BuildConfig.DEVELOPER_ACCESS_SHA256.equals(sha256(entered))){status.setText("Incorrect developer password.");status.setTextColor(BAD);pass.setText("");return;}resolveContextAndOpen();});setContentView(s);s.requestApplyInsets();}
