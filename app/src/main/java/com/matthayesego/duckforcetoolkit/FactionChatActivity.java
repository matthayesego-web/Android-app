...
/** TornFCA app-native faction chat with cache-first channels and device-local user blocking. */
public class FactionChatActivity extends Activity {
    ...
    private void bootstrap(){
        ...
        new Thread(()->{try{
            ...
            runOnUiThread(()->{if(cached!=null)render(cached,"general");load(cached==null);});
    ...
    private void load(boolean full){
        ...
        String key=new SecureApiKeyStore(this).load();if(key==null)return;
        loading=true;refreshing=true;String channel=selectedChannel();runOnUiThread(this::updateFreshness);
        new Thread(()->{try{
            JSONArray messages=CommunityBackendClient.getChatMessages(key,channel);StartupWarmCache.putChat(session.factionId,channel,messages);String signature=messages.toString();
            runOnUiThread(()->{refreshing=false;if(full||!signature.equals(lastRenderedSignature))render(messages,channel);else updateFreshness();});
        }catch(Exception e){runOnUiThread(()->{refreshing=false;updateFreshness();if(full&&lastRenderedSignature.isEmpty())renderError(e.getMessage()==null?"Unable to load faction chat.":e.getMessage());else Toast.makeText(this,"Chat refresh failed. Cached messages remain available.",Toast.LENGTH_SHORT).show();});}
    ...
    private void openChannel(String channel){
        ...
        if(cached!=null)render(cached,channel);
        load(cached==null);
    }

    private void updateFreshness(){if(freshnessView!=null&&session!=null)freshnessView.setText(DataFreshness.label(StartupWarmCache.chatAgeMs(session.factionId,selectedChannel()),refreshing));}

    private void render(JSONArray messages,String selected){
        if(session==null)return;
        lastRenderedSignature=messages==null?"[]":messages.toString();
        ...
        freshnessView=TornFcaUi.text(this,DataFreshness.label(StartupWarmCache.chatAgeMs(session.factionId,selected),refreshing),11.5f,TornFcaUi.MUTED,false);LinearLayout.LayoutParams fsp=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,ViewGroup.LayoutParams.WRAP_CONTENT);fsp.bottomMargin=TornFcaUi.dp(this,8);r.addView(freshnessView,fsp);
        ...
        int visible=0;
        JSONArray safeMessages=messages==null?new JSONArray():messages;
        for(int i=0;i<safeMessages.length();i++){
            JSONObject m=safeMessages.optJSONObject(i);if(m==null)continue;
