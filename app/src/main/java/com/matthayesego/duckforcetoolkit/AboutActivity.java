...
public class AboutActivity extends Activity {
    ...
    private void render(){
        ...
        add(r,card("One-key access model","The recommended key is one standard 16-character Limited Access Torn API key. Full Access is not required for ordinary TornFCA use. Private faction-wide features additionally depend on Torn's in-game Faction API Access permission for the player's faction position; that is a faction permission, not a stronger API-key level.",GREEN));
        add(r,card("Torn API use & request safety","Session-only keys remain in app memory for the current session. If the player explicitly chooses 7, 30 or 90 day retention, the key is AES-GCM encrypted on-device using Android Keystore and expires automatically. Direct Torn calls share one throttled boundary with a small interactive burst and a sustained rate of roughly 30 requests/minute, plus caching and bounded retry/backoff for transient failures.",GREEN));
        add(r,card("Shared TornFCA backend","Shared notices, banking/history and premium entitlement are dormant unless a TornFCA backend URL is configured. When configured, a shared faction request may temporarily transmit the player's key over HTTPS so the backend can verify identity and faction permissions against Torn; the design does not persist that key. Shared application records are scoped by faction_id and player_id.",GOLD));
        ...
        add(r,card("Premium entitlement data","Premium status is intended to be keyed to Torn player_id and verified server-side when the premium backend is configured. The Android app does not claim or process Torn cash/item transfers and does not treat a local premium flag as authoritative.",GOLD));
        add(r,card("Android security","Only Internet permission is requested. Cleartext traffic is disabled, application backup is disabled, non-launcher activities are not exported, and retained Torn keys remain encrypted locally rather than being written in plaintext preferences.",GREEN));
