...
/** Member-safe territory ownership and territory-war view using Torn API v2 public faction data. */
public class TerritoryWarActivity extends Activity {
    ...
    private String outcome(String result,boolean aggressor){if("success_assault".equals(result))return aggressor?"WIN":"LOSS";if("fail_assault".equals(result))return aggressor?"LOSS":"WIN";return"RESULT";}
    private String humanResult(String raw){if(raw==null||raw.isBlank())return"Completed";String[] bits=raw.replace('_',' ').split(" ");StringBuilder b=new StringBuilder();for(String bit:bits){if(bit.isBlank())continue;if(b.length()>0)b.append(' ');b.append(Character.toUpperCase(bit.charAt(0))).append(bit.substring(1));}return b.toString();}
