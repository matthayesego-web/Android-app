...
public class PremiumPreviewActivity extends Activity {
    ...
    private void render(){ScrollView s=shell();LinearLayout r=new LinearLayout(this);r.setOrientation(LinearLayout.VERTICAL);s.addView(r,new ScrollView.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,ViewGroup.LayoutParams.WRAP_CONTENT));Button back=button("← Companion");back.setOnClickListener(v->finish());r.addView(back,new LinearLayout.LayoutParams(dp(124),dp(44)));TextView title=text("Premium Preview",27,TEXT,true);LinearLayout.LayoutParams tp=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,ViewGroup.LayoutParams.WRAP_CONTENT);tp.topMargin=dp(14);r.addView(title,tp);TextView state=text(PremiumEntitlementStore.summary(this),12,GOLD,true);LinearLayout.LayoutParams sp=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,ViewGroup.LayoutParams.WRAP_CONTENT);sp.topMargin=dp(4);sp.bottomMargin=dp(14);r.addView(state,sp);addCard(r,card("Smart Alerts","Optional Android alerts for war starts, chain danger, OC ready states and personal participation reminders.",BLUE));addCard(r,card("Advanced Member Intel","Longer activity history, participation trends, member comparison and faster leadership follow-up.",GOLD));addCard(r,card("War Analytics","Zero-hit watch, participation trends, readiness snapshots and deeper completed-war reporting.",GOLD));addCard(r,card("History & Export","Extended history, copy-ready reports and export tools beyond the quick live dashboard.",BLUE));addCard(r,card("Automatic Unlock Architecture","Future entitlement will be tied to Torn player ID. A backend will verify the in-game payment/receipt first, then the app will cache the verified premium grant. No client-side payment claim will unlock features.",GOOD));addCard(r,card("Not For Sale Yet","Premium is only a prepared framework right now. No in-game payment is required or accepted by this build.",BORDER));setContentView(s);s.requestApplyInsets();}
}
