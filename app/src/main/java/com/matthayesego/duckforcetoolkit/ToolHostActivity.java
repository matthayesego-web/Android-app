...
public class ToolHostActivity extends Activity {
    ...
    private enum Tool {
        AUDITOR("Faction Xanax Auditor", "auditor.duckforce.app", "xanax_auditor.html"),
        ARMORY("Xanax Armory Log", "armory.duckforce.app", "armory_log.html"),
        TRAIN("Company Train Calculator", "train.duckforce.app", "train_calculator.html");

        final String title;
        final String domain;
        final String asset;
        Tool(String title, String domain, String asset) {
            this.title = title;
            this.domain = domain;
            this.asset = asset;
        }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Tool tool;
        try {
            tool = Tool.valueOf(getIntent().getStringExtra(EXTRA_TOOL));
        } catch (Exception e) {
            finish();
            return;
        }
    ...
    private void applyInsets(LinearLayout view) {
    ...
    private void openTool(Tool tool) {
        ...
        toolbar.setPadding(dp(8), dp(7), dp(12), dp(7));
        toolbar.setBackgroundColor(PANEL);

        Button back = new Button(this);
        back.setText("← Companion");
        back.setAllCaps(false);
        back.setTextColor(TEXT);
        back.setBackgroundColor(Color.TRANSPARENT);
        back.setOnClickListener(v -> finish());
        toolbar.addView(back, new LinearLayout.LayoutParams(dp(118), dp(44)));
        ...
        webView.setWebViewClient(new WebViewClient() {
            ...
            public WebResourceResponse shouldInterceptRequest(WebView view, WebResourceRequest request) {
                ...
                if ("api.torn.com".equalsIgnoreCase(uri.getHost()) && "GET".equalsIgnoreCase(request.getMethod())) {
                    WebResourceResponse response = proxyTornApi(uri);
            ...
            public void onPageFinished(WebView view, String url) {
                ...
                if (url != null && url.contains(tool.domain)) injectKey(view);
            }
        });
    ...
    private void injectKey(WebView view) {
        String key = keyStore.load();
        if (key == null || key.isEmpty()) return;
        String js = "(function(){var e=document.getElementById('key');if(e){e.value=" + JSONObject.quote(key)
                + ";e.readOnly=true;e.placeholder='Using Duck Force Companion API key';}})();";
        ...
        try {
            ...
            InputStream stream = new FilterInputStream(raw) {
                @Override public void close() throws IOException {
                    try { super.close(); } finally { finalConnection.disconnect(); }
                }
            };
            ...
            return new WebResourceResponse("application/json", "UTF-8", status, reason, headers, stream);
        } catch (Exception e) {
