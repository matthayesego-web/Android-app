...
public final class NotificationCenter {
    ...
    private static final String DEDUPE_PREFS="tornfca_notification_dedupe_v1",DEDUPE_KEY="last_key",DEDUPE_AT="last_at";
    ...
    public static void receive(Context c,String type,String title,String body,int factionId){
        if(c==null)return;
        String cleanType=type==null?"personal":type,cleanTitle=title==null?"TornFCA":title,cleanBody=body==null?"":body;
        if(isRapidDuplicate(c,cleanType,cleanTitle,cleanBody,factionId))return;
        NotificationInboxStore.add(c,cleanType,cleanTitle,cleanBody,factionId);
        if(!AppSettingsStore.categoryEnabled(c,cleanType)||!canPost(c))return;
        ensureChannels(c);String channel=channel(cleanType);Intent open=targetIntent(c,cleanType,factionId);open.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK|Intent.FLAG_ACTIVITY_CLEAR_TOP);PendingIntent pi=PendingIntent.getActivity(c,Math.abs((cleanTitle+cleanBody+System.nanoTime()).hashCode()),open,PendingIntent.FLAG_UPDATE_CURRENT|PendingIntent.FLAG_IMMUTABLE);Notification.Builder b=Build.VERSION.SDK_INT>=26?new Notification.Builder(c,channel):new Notification.Builder(c);b.setSmallIcon(android.R.drawable.ic_dialog_info).setContentTitle(cleanTitle).setContentText(cleanBody).setStyle(new Notification.BigTextStyle().bigText(cleanBody)).setAutoCancel(true).setContentIntent(pi).setCategory(Notification.CATEGORY_REMINDER);if("banking".equalsIgnoreCase(cleanType)&&Build.VERSION.SDK_INT<26)b.setPriority(Notification.PRIORITY_HIGH);((NotificationManager)c.getSystemService(Context.NOTIFICATION_SERVICE)).notify((int)(System.currentTimeMillis()&0x7fffffff),b.build());}
    private static boolean isRapidDuplicate(Context c,String type,String title,String body,int factionId){String key=(type+"\n"+factionId+"\n"+title+"\n"+body);long now=System.currentTimeMillis();SharedPreferences p=c.getSharedPreferences(DEDUPE_PREFS,Context.MODE_PRIVATE);String previous=p.getString(DEDUPE_KEY,"");long previousAt=p.getLong(DEDUPE_AT,0L);if(key.equals(previous)&&now-previousAt>=0L&&now-previousAt<DEDUPE_WINDOW_MS)return true;p.edit().putString(DEDUPE_KEY,key).putLong(DEDUPE_AT,now).apply();return false;}
