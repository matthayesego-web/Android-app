...
public class FeatureRouterActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle savedInstanceState){super.onCreate(savedInstanceState);getWindow().setStatusBarColor(BG);getWindow().setNavigationBarColor(BG);keyStore=new SecureApiKeyStore(this);DeveloperSettings.activityMaxPages(this);route();}
    ...
    private void route(){
        ...
        if(key==null||key.trim().isEmpty()){showStatus("No saved Torn API key is available. Return to TornFCA and sign in again.",BAD);return;}
