...
/** Hidden owner-only controls for the global premium entitlement backend. */
public class PremiumAdminActivity extends Activity {
    ...
    private EditText field(String hint,boolean number){EditText e=new EditText(this);e.setHint(hint);e.setHintTextColor(MUTED);e.setTextColor(TEXT);e.setSingleLine(true);e.setPadding(dp(13),0,dp(13),0);e.setBackground(rounded(BG,BORDER,11));if(number)e.setInputType(InputType.TYPE_CLASS_NUMBER);return e;}
    ...
    private void render(){
        ...
        LinearLayout auth=card(GOLD);auth.addView(text("Owner authorization",18,TEXT,true));auth.addView(text("Server changes are authorized by your verified Torn owner identity. TornFCA sends the saved API key only to verify the request; the premium backend rejects admin changes from any other Torn player and does not store the client key.",12,MUTED,false));add(root,auth);

        LinearLayout pricing=card(BLUE);pricing.addView(text("Xanax conversion",18,TEXT,true));pricing.addView(text("Launch pricing is 1 Xanax = 7 Premium days. Incoming Xanax is detected from the owner's official Torn item-receive log; duplicate logs can never grant twice.",12,MUTED,false));EditText days=field("Days of Premium per Xanax",true);days.setText("7");LinearLayout.LayoutParams dpv=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,dp(48));dpv.topMargin=dp(10);pricing.addView(days,dpv);EditText message=field("Required transfer message (blank = any Xanax)",false);message.setText("TORNFCA");LinearLayout.LayoutParams mp=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,dp(48));mp.topMargin=dp(8);pricing.addView(message,mp);Button save=button("Update Premium Conversion",BLUE);LinearLayout.LayoutParams svp=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,dp(48));svp.topMargin=dp(9);pricing.addView(save,svp);save.setOnClickListener(v->{int d=parseInt(days.getText().toString(),7);runAdmin((key)->PremiumBackendClient.updateConfig(key,d,message.getText().toString()),"Premium conversion updated.");});add(root,pricing);

        LinearLayout grant=card(GREEN);grant.addView(text("Complimentary Premium",18,TEXT,true));grant.addView(text("Gift Premium to any Torn player without requiring a Xanax payment. This owner-only action uses the same server-verified entitlement as paid Premium, so it follows the player's Torn ID across factions and expires normally.",12,MUTED,false));EditText player=field("Torn player ID",true);LinearLayout.LayoutParams gp=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,dp(48));gp.topMargin=dp(10);grant.addView(player,gp);EditText grantDays=field("Complimentary days to grant",true);grantDays.setText("30");LinearLayout.LayoutParams gdp=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,dp(48));gdp.topMargin=dp(8);grant.addView(grantDays,gdp);Button give=button("Grant Complimentary Premium",GREEN);LinearLayout.LayoutParams gbp=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,dp(48));gbp.topMargin=dp(9);grant.addView(give,gbp);give.setOnClickListener(v->{int id=parseInt(player.getText().toString(),0),d=parseInt(grantDays.getText().toString(),0);if(id<=0||d<=0){Toast.makeText(this,"Enter a valid player ID and number of days.",Toast.LENGTH_SHORT).show();return;}runAdmin((key)->PremiumBackendClient.grantComplimentary(key,id,d),"Complimentary Premium granted.");});add(root,grant);

        LinearLayout note=card(BORDER);note.addView(text("Activation paths",17,TEXT,true));note.addView(text("Paid: player transfers Xanax in Torn → owner log 4103 appears → backend scanner validates item #206 and sender → entitlement expiry is extended.\n\nComplimentary: owner enters a Torn player ID and number of days above → the backend extends the same entitlement without a payment.\n\nEvery admin mutation is re-authorized by the backend against the signed-in Torn owner ID; no developer password is required and no client can query another player's entitlement.",12,MUTED,false));add(root,note);
