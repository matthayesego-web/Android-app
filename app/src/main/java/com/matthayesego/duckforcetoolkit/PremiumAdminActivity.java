...
public class PremiumAdminActivity extends Activity {
    ...
    private void render(){
        ...
        LinearLayout auth=card(GOLD);auth.addView(text("Developer authorization",18,TEXT,true));auth.addView(text("Re-enter the developer password for server-side changes. TornFCA does not save the plaintext password.",12,MUTED,false));EditText password=field("Developer password",true,false);LinearLayout.LayoutParams pp=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,dp(50));pp.topMargin=dp(10);auth.addView(password,pp);add(root,auth);

        LinearLayout pricing=card(BLUE);pricing.addView(text("Xanax conversion",18,TEXT,true));pricing.addView(text("Incoming Xanax is detected from the owner's official Torn item-receive log. One backend scan per minute; duplicate logs can never grant twice.",12,MUTED,false));EditText days=field("Days of Premium per Xanax",false,true);days.setText("15");LinearLayout.LayoutParams dpv=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,dp(48));dpv.topMargin=dp(10);pricing.addView(days,dpv);EditText message=field("Required transfer message (blank = any Xanax)",false,false);message.setText("TORNFCA");LinearLayout.LayoutParams mp=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,dp(48));mp.topMargin=dp(8);pricing.addView(message,mp);Button save=button("Update Premium Conversion",BLUE);LinearLayout.LayoutParams svp=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,dp(48));svp.topMargin=dp(9);pricing.addView(save,svp);save.setOnClickListener(v->{int d=parseInt(days.getText().toString(),15);runAdmin(()->PremiumBackendClient.updateConfig(password.getText().toString(),d,message.getText().toString()),"Premium conversion updated.");});add(root,pricing);

        LinearLayout grant=card(GREEN);grant.addView(text("Manual / beta grant",18,TEXT,true));grant.addView(text("Grant Premium by Torn player ID for testing, support or a manual correction. This uses the same server entitlement the automatic Xanax scanner uses.",12,MUTED,false));EditText player=field("Torn player ID",false,true);LinearLayout.LayoutParams gp=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,dp(48));gp.topMargin=dp(10);grant.addView(player,gp);EditText grantDays=field("Days to grant",false,true);grantDays.setText("30");LinearLayout.LayoutParams gdp=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,dp(48));gdp.topMargin=dp(8);grant.addView(grantDays,gdp);Button give=button("Grant Premium",GREEN);LinearLayout.LayoutParams gbp=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,dp(48));gbp.topMargin=dp(9);grant.addView(give,gbp);give.setOnClickListener(v->{int id=parseInt(player.getText().toString(),0),d=parseInt(grantDays.getText().toString(),0);if(id<=0||d<=0){Toast.makeText(this,"Enter a valid player ID and days.",Toast.LENGTH_SHORT).show();return;}runAdmin(()->PremiumBackendClient.grant(password.getText().toString(),id,d),"Premium grant applied.");});add(root,grant);

        LinearLayout note=card(BORDER);note.addView(text("Activation path",17,TEXT,true));note.addView(text("Sender transfers Xanax in Torn → owner log 4103 appears → one-minute backend scanner validates item #206 and sender → entitlement expiry is extended → TornFCA refreshes the player's entitlement cache. No player API key is used to watch the payment.",12,MUTED,false));add(root,note);
        setContentView(s);s.requestApplyInsets();
    }

    private interface AdminCall{Object run()throws Exception;}
    private void runAdmin(AdminCall call,String success){if(!PremiumBackendClient.isConfigured()){Toast.makeText(this,"Premium backend is not configured in this build.",Toast.LENGTH_LONG).show();return;}new Thread(()->{try{call.run();runOnUiThread(()->Toast.makeText(this,success,Toast.LENGTH_LONG).show());}catch(Exception e){String m=e.getMessage()==null?"Premium admin action failed.":e.getMessage();runOnUiThread(()->Toast.makeText(this,m,Toast.LENGTH_LONG).show());}}).start();}
