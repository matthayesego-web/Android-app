...
public class LegalDocumentActivity extends Activity {
    ...
    private void renderPrivacy(LinearLayout r){
        ...
        section(r,"Data stored on your device","Depending on the features you use, TornFCA may store preferences, notification history, provider-consent choices, cached faction identity, onboarding/checklist state, war-prep state, personal training baselines and entitlement state. Personal training baselines are device-local and are scoped to the player and faction.");
        ...
        section(r,"Notifications","TornFCA uses Firebase Cloud Messaging when cloud push is enabled. A device push token and verified player/faction scope may be registered so relevant notifications can reach your device. TornFCA does not use Firebase Analytics in the current Android build.");
        ...
        section(r,"Changes and contact","Material changes to these disclosures will use a new legal version and can require renewed in-app acknowledgement. Privacy questions or deletion requests can be sent through the developer contact published with TornFCA on its distribution page or Google Play listing.");
    }
    ...
    private void renderTerms(LinearLayout r){
        ...
        section(r,"Community conduct","Do not use TornFCA to harass others, distribute unlawful or malicious content, evade access controls, attack services, impersonate another player, or deliberately interfere with another user's access. Faction/community content may be removed or restricted when reasonably necessary for safety, security or service operation.");
