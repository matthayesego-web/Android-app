...
public class WarNoticeActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        boolean requestedPublish = getIntent().getBooleanExtra(EXTRA_CAN_PUBLISH, false);
        canPublish = false;
        if (!requestedPublish || MemberPresentationPolicy.memberPreview(this)) { startScreen(); return; }
        String key=keyStore.load();
        if(key==null||key.isBlank()){startScreen();return;}
        new Thread(()->{try{AuthSession verified=TornApiClient.authenticateFreshFaction(key);canPublish=AccessPolicy.isLeaderPosition(verified.position);}catch(Exception ignored){canPublish=false;}runOnUiThread(this::startScreen);},"TornFCA-NoticePublishCheck").start();
    }

    private void startScreen(){buildUi();refreshAll();handler.removeCallbacks(ticker);handler.post(ticker);}
    ...
    private void buildUi() {
        ...
        TextView title=text("Faction Notices",27,TEXT,true);LinearLayout.LayoutParams tp=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,ViewGroup.LayoutParams.WRAP_CONTENT);tp.topMargin=dp(14);tp.bottomMargin=dp(12);root.addView(title,tp);
        ...
        TextView ntitle=text("FACTION MESSAGE BOARD",12,MUTED,true);ntitle.setLetterSpacing(.08f);LinearLayout.LayoutParams np=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,ViewGroup.LayoutParams.WRAP_CONTENT);np.topMargin=dp(18);np.bottomMargin=dp(8);root.addView(ntitle,np);
        notices=new LinearLayout(this);notices.setOrientation(LinearLayout.VERTICAL);root.addView(notices);

        if(canPublish){
            LinearLayout publish=card(BLUE);publish.addView(text("Post faction notice",18,TEXT,true));
    ...
    private void loadNotices(){
        ...
        if(!CompanionBackendClient.isConfigured()){addNoticeCard("Notice board ready","Shared backend deployment is still pending. War countdown works now; faction-wide notices will switch on when the backend URL is connected.","",BORDER);return;}
