...
(() => {
  ...
  function configured() {
    return /^https:\/\//i.test(BACKEND_URL) && !BACKEND_URL.includes('###') && LISTENER_TOKEN && !LISTENER_TOKEN.includes('###');
  }
  function normalize(text) { return String(text || '').replace(/\s+/g, ' ').trim(); }
  function isBankingRequest(text) {
    const clean = normalize(text);
    return !!clean && clean.length <= 700 && REQUEST_WORDS.test(clean) && REQUEST_PATTERNS.some(p => p.test(clean));
  }
  function parseAmount(text) {
    ...
    const patterns = [
      ...
      /\$\s*(\d{4,})\b/i,
      /(?:!bank|bank|withdraw|send|get)\s+(?:me\s+)?(\d{4,})\b/i
    ];
    for (const pattern of patterns) {
      const m = clean.match(pattern); if (!m) continue;
      let v = Number(m[1]); if (!Number.isFinite(v) || v <= 0) continue;
      const suffix = String(m[2] || '').toLowerCase();
      if (suffix.startsWith('b')) v *= 1_000_000_000;
      else if (suffix.startsWith('m')) v *= 1_000_000;
      else if (suffix.startsWith('k') || suffix.startsWith('t')) v *= 1_000;
      return Math.round(v);
    }
    return null;
  }
  ...
  function factionChatIsOpen() {
    const selectors = ['[role="tab"][aria-selected="true"]','[class*="chat"] [class*="active"]','[class*="chat"] [class*="title"]','[class*="chat"] [class*="header"]','[data-channel]','[data-chat-type]'];
    for (const selector of selectors) {
      let rows=[]; try { rows=document.querySelectorAll(selector); } catch (_) {}
  ...
  function activeSession() {
    return configured() && !document.hidden && document.hasFocus() && Date.now() <= activeUntil && factionChatIsOpen();
  }
  ...
  function profileIdFrom(node) {
    ...
    for(const a of anchors){const href=a.getAttribute('href')||'';const m=href.match(/[?&#]XID=(\d+)/i);if(m)return{id:Number(m[1]),name:normalize(a.textContent)};}
    return null;
  }
  function messageTimestamp(node) {
    const time=node.querySelector&&node.querySelector('time[datetime]');if(time){const p=Date.parse(time.getAttribute('datetime'));if(Number.isFinite(p))return Math.floor(p/1000);}
    const t=node.querySelector&&node.querySelector('[data-timestamp]');if(t){const raw=Number(t.getAttribute('data-timestamp'));if(Number.isFinite(raw)&&raw>0)return raw>1e12?Math.floor(raw/1000):Math.floor(raw);}
    return 0;
  }
  ...
  function candidateMessages() {
    const nodes=new Set();
    for(const selector of ['[data-message-id]','[data-chat-message-id]','[class*="chatMessage"]','[class*="chat-message"]','[class*="message_"]','[class*="message-"]']){try{document.querySelectorAll(selector).forEach(n=>nodes.add(n));}catch(_){}}
    document.querySelectorAll('a[href*="XID="]').forEach(a=>{const p=a.closest('[data-message-id],[data-chat-message-id],[class*="message"],li');if(p)nodes.add(p);});
    return [...nodes].filter(node=>{if(!visible(node))return false;const text=normalize(node.innerText||node.textContent);return text.length>=3&&text.length<=900&&REQUEST_WORDS.test(text);});
  }

  async function submitRequest(payload) {
    if(!activeSession())return false;
    try {
      await fetch(BACKEND_URL,{method:'POST',mode:'no-cors',cache:'no-store',headers:{'Content-Type':'text/plain;charset=UTF-8'},body:JSON.stringify({action:'listener_event',listener_token:LISTENER_TOKEN,listener_version:VERSION,...payload})});
      return true;
    } catch(error) { console.warn('[TornFCA Banking] Queue failed:',error); return false; }
  }
  ...
  async function scan() {
    ...
    for(const node of candidateMessages()){
      if(!activeSession())return;
      const text=normalize(node.innerText||node.textContent);if(!isBankingRequest(text))continue;
      const profile=profileIdFrom(node);if(!profile||!profile.id)continue;
      const messageId=explicitMessageId(node),timestamp=messageTimestamp(node);
      const fingerprint=messageId?`msg:${messageId}`:`fp:${hash(`${profile.id}|${timestamp}|${text.toLowerCase()}`)}`;
      if(wasSeen(fingerprint)||inflight.has(fingerprint))continue;inflight.add(fingerprint);
      try {
        const amount=parseAmount(text);
        const sent=await submitRequest({fingerprint,message_id:messageId,requester_id:profile.id,requester_name:profile.name||`ID ${profile.id}`,request_text:text,requested_amount:amount,request_mode:amount?'AMOUNT':'FULL_BALANCE',message_timestamp:timestamp||null,detected_timestamp:Math.floor(Date.now()/1000),source:'FACTION_CHAT_ACTIVE'});
